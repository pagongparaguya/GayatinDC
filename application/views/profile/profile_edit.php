	...
	<legend>Edit Profile</legend>
		...
		<div class="row clearfix">
  		...
  		<?php foreach($profile as $prof):?>
  			...
  			<input class="form-control" value="<?php echo $prof->firstname;?>" name="fname" type="text" placeholder="First Name" required/><br>
  			<input class="form-control" value="<?php echo $prof->lastname;?>" name="lname" type="text" placeholder="Last Name" required/><br>
  			<input class="form-control" value="<?php echo $prof->middlename;?>" name="mname" type="text" placeholder="Middle Name" required/><br>
  			<input class="form-control" value="<?php echo $prof->address;?>" name="hadd" type="text" placeholder="Home Address" required/><br>
  			<input class="form-control" value="<?php echo $prof->telno;?>" name="htno" type="text" placeholder="Tel. No." /><br>
  			<input class="form-control" value="<?php echo $prof->contactno;?>" name="mno" type="text" placeholder="Mobile No." pattern="[0][9][0-9]{9}" required/><br>
			<div class="col-md-6 columns">
  		...
  		<div class="col-md-6 columns">
  			...
  			<input class="form-control" value="<?php echo $prof->officeaddress;?>" name="oadd" type="text" placeholder="Office Address" /><br>
  			<input class="form-control" value="<?php echo $prof->officetelno;?>" name="otno" type="text" placeholder="Tel. No." /><br>
  		</div>
	</div>
	<br><br>
	...
	<div class="row">
		<div id="dtcon" class="col-md-6 columns">
  			...
  			<?php $rctr=0;foreach($chronicailment as $ca):$rctr+=1;?>
  				...
  				<input id="ca<?php echo $rctr;?>" class="form-control" value="<?php echo $ca->chronicailmentname;?>" name="ca<?php echo $rctr;?>" placeholder="Chronic Ailment (<?php echo $rctr;?>)"/><br>
  			<?php endforeach;?>
            ...
            <button id="rmv_chrail" type="button" class="btn btn-danger">Remove</button><br><br>
  			...
  			<?php $cctr=0;foreach($drugstaken as $dt):$cctr+=1;?>
  				...
  				<textarea id="dt<?php echo $cctr;?>" class="form-control" name="dt<?php echo $cctr;?>" placeholder="Drugs Taken (<?php echo $cctr;?>)"><?php echo $dt->drugname;?></textarea><br>
  			<?php endforeach;?>
  		...
  		<div id="alcon" class="col-md-6 columns">
  			<input id="al1" class="form-control" value="<?php echo $prof->bloodpressure;?>" name="bp" type="text" placeholder="Blood Pressure" /><br>
  			<?php $actr=0;foreach($allergies as $al):$actr+=1;?>
  				<input type="hidden" name="allergyid<?php echo $actr;?>" value="<?php echo $al->id; ?>"/>
  				<textarea id="al<?php echo $actr;?>" class="form-control" name="al<?php echo $actr;?>" placeholder="Allergies (<?php echo $actr;?>)"><?php echo $al->allergyname;?></textarea><br>
  			<?php endforeach;?>
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
                $("#add_drugs").click(function(){
                	ctemp = cctr;
                	cctr+=1;
                    $('<br id="bdt' + cctr + '"><textarea id="dt' + cctr + '" class="form-control" name="adt[]" placeholder="Drugs Taken ('+ cctr +')"></textarea>').insertAfter("#dt"+ ctemp);
                });
                ...
                $("#add_allergy").click(function(){
                	atemp = actr;
                	actr+=1; 
                    $('<br id="bal' + actr + '"><textarea id="al' + actr + '" class="form-control" name="aal[]" placeholder="Allergies ('+ actr +')"></textarea>').insertAfter("#al"+ atemp);                
                });
                ...
                $("#add_chrail").click(function(){
                	rtemp = rctr;
                	rctr+=1;
                    $('<br id="bca' + rctr + '"><input id="ca' + rctr + '" class="form-control" name="aca[]" placeholder="Chronic Ailments ('+ rctr +')"/>').insertAfter("#ca"+ rtemp);
                });
